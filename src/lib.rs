// physics modules
pub mod steady_diff;
pub mod steady_diff_multi;
pub mod steady_diffadv;
pub mod steady_diffadv_multi;
pub mod transient_diff;
pub mod transient_diff_multi;
pub mod transient_diffadv;
pub mod transient_diffadv_multi;

// physics re-exports
pub use crate::steady_diff::*;
pub use crate::steady_diff_multi::*;
pub use crate::steady_diffadv::*;
pub use crate::steady_diffadv_multi::*;
pub use crate::transient_diff::*;
pub use crate::transient_diff_multi::*;
pub use crate::transient_diffadv::*;
pub use crate::transient_diffadv_multi::*;

// main modules
pub mod domain_0d;
pub mod domain_1d;
pub mod mesh_1d;
pub mod problem_1d;
pub mod scalar_0d;
pub mod scalar_1d;
pub mod script_steady;
pub mod script_transient;
pub mod steady_base;
pub mod transient_base;
pub mod utils_csv;
pub mod utils_error;
pub mod utils_limiter;
pub mod variable_1d;

// main re-exports
pub use crate::domain_0d::*;
pub use crate::domain_1d::*;
pub use crate::mesh_1d::*;
pub use crate::problem_1d::*;
pub use crate::scalar_0d::*;
pub use crate::scalar_1d::*;
pub use crate::script_steady::*;
pub use crate::script_transient::*;
pub use crate::steady_base::*;
pub use crate::transient_base::*;
pub use crate::utils_error::*;
pub use crate::utils_limiter::*;
pub use crate::variable_1d::*;
