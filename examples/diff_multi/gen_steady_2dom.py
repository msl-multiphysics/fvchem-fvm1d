...
# settings
x_min = 0.0
dx_list = [0.5, 0.5]  # lengths of segments
nc_list = [10, 20]  # number of cells

# create input file directory
os.makedirs('fvchem_fvm1d/examples/input_mesh_2dom', exist_ok=True)
...
pd.DataFrame({'fid': face_id, 'x': face_x}).to_csv('fvchem_fvm1d/examples/input_mesh_2dom/mesh_face.csv', index=False)
...
pd.DataFrame({'cid': cell_id, 'x': cell_x, 'dx': cell_dx}).to_csv('fvchem_fvm1d/examples/input_mesh_2dom/mesh_cell.csv', index=False)
...
pd.DataFrame({'cid': cell_id, 'nid_0': cc_nid0, 'nid_1': cc_nid1, 'dist_0': cc_dist0, 'dist_1': cc_dist1}).to_csv('fvchem_fvm1d/examples/input_mesh_2dom/mesh_cell_cell.csv', index=False)
...
pd.DataFrame({'cid': cell_id, 'fid_0': cf_fid0, 'fid_1': cf_fid1, 'dist_0': cf_dist0, 'dist_1': cf_dist1, 'norm_0': cf_norm0, 'norm_1': cf_norm1}).to_csv('fvchem_fvm1d/examples/input_mesh_2dom/mesh_cell_face.csv', index=False)
...
pd.DataFrame({'fid': face_id, 'cid_0': fc_cid0, 'cid_1': fc_cid1, 'dist_0': fc_dist0, 'dist_1': fc_dist1}).to_csv('fvchem_fvm1d/examples/input_mesh_2dom/mesh_face_cell.csv', index=False)
...
for i in range(len(nc_list)):
    ...
    start_idx = end_idx
pd.DataFrame({'rid': reg_id, 'cid': cell_id}).to_csv('fvchem_fvm1d/examples/input_mesh_2dom/mesh_reg.csv', index=False)
...
pd.DataFrame({'bid': bnd_id, 'cid': bnd_cid, 'loc': bnd_loc}).to_csv('fvchem_fvm1d/examples/input_mesh_2dom/mesh_bnd.csv', index=False)
