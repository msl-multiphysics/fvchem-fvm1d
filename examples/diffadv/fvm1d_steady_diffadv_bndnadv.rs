...
fn main() {
    ...
    let dom_r = Problem1D::add_dom0d(&mut prob, dom, 49, 1).unwrap();

    // add properties
    create_dir_all("examples/output_steady_diffadv_bndnadv").unwrap();
    let c = Problem1D::add_var1d(
        &mut prob,
        dom,
        1.0,
        "examples/output_steady_diffadv_bndnadv/c".to_string(),
        0,
    ).unwrap();
    let d = Problem1D::add_scl1d(&mut prob, dom, 0.10, "".to_string(), 0).unwrap();
    let u = Problem1D::add_scl1d(&mut prob, dom, 2.0, "".to_string(), 0).unwrap();
    let r = Problem1D::add_scl1d(&mut prob, dom, 0.0, "".to_string(), 0).unwrap();
    ...
    solver.add_boundary_fluxnoadvection(dom_r, n_r);
